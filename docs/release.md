...
| Commit + tag                      | GPG-signs the bump commit and pushes the `vX.Y.Z` tag. The tag push simultaneously triggers `release-push-image.yaml`.                                                                          |
...
2. **General Availability** — Once testing passes, trigger `GA` from `release/X.Y`.
